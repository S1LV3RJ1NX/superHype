...
- Seed data is a separate idempotent step, not baked into the schema migration: `app/seed.py` bootstraps admin users. (The default-writing-skill seed from earlier drafts was removed when the writing-skill feature was retired in the interaction-first reframe.)
